...
class AgentRouterService:
    ...
    def handle(self, request: AgentRouterInput) -> AgentRouterOutput:
        ...
        self.store.record_session_start(request.session_id, locale=language, consent=self.store.session_has_consent(request.session_id))

        consent = self.store.session_has_consent(request.session_id)
        if not consent:
        ...
        cache_key = self._cache_key(translated_query.text, filters, mode, language)
        ...
        if cached_payload is not None:
            ...
            self._record_event(request, output, payload={"cache": "hit"}, include_content=True)
        ...
        self._record_event(request, output, payload={"cache": "miss"}, include_content=True)
        self._record_qa(request, translated_query.text, output)
